...
class CANPackerPanda(CANPackerSafety):
  def make_can_msg_panda(self, name_or_addr, bus, values, fix_checksum=None):
    return self.make_can_msg_safety(name_or_addr, bus, values, fix_checksum=fix_checksum)
  ...
  def test_gas_interceptor_safety_check(self):
    for gas in np.arange(0, 4000, 100):
      for controls_allowed in [True, False]:
        ...
        self.assertEqual(should_tx, self._tx(self._interceptor_gas_cmd(gas)))
